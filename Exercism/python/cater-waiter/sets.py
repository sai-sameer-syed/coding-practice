...
def tag_special_ingredients(dish):
    ...
    for each in ingredients:
        if each in SPECIAL_INGREDIENTS:
            special_ings.add(each)
    return (name, special_ings)
...
def singleton_ingredients(dishes, intersection):
    ...
    for ingredient in all_ingredients:
        count = 0
        for dish in dishes:
            if ingredient in dish and ingredient not in intersection:
                count += 1
        if count == 1:
